...
void Reasoning::Input()
{
    ...
    cout << endl << "请输入：";
    getline(cin, input);
    // input = "F(A,B,C,D)=∑ m(0,2,3,5,6,7,8,9,13,14)"; // !测试用例
    ...
    {
        cout << "支持的逻辑运算：否定:~、合取:^、析取:v/否定:`(兼容'或‘或’)、合取:*、析取:+、条件:>、双条件:<、异或:@、与非:[、或非:]" << endl;
        cout << "命题变元不区分大小写,只能为A-Z除V外的字母,兼容中文括号，支持离散数学和数字逻辑两种符号体系，请勿混用" << endl;
        cout << "命令：输入/end或/exit结束,/setting设置输出选项,/truthtable或/t用真值表形式输入,/clear清空页面,/help帮助" << endl;
        cout << "功能：输出真值表、主合取析取范式、卡诺图、卡诺图化简" << endl;
        cout << "示例输入:" << endl;
        cout << "~(~A^Bv(A^~BVc)^A>C)>(Cv(A<B))" << endl;
        cout << "(A`(B+C`))`(A+B`+C) 或 (A`(B+C`))`*(A+B`+C)" << endl;
        cout << "F(x,y,z)=∑ m(2,3,5,7)或F(x,y,z)=∏ M(2,3,5,7)" << endl;
        cout << "F(A,B,C,D)=∑ m(0,2,3,5,6,7,8,9)+ ∑ d(10,11,12,13,14,15)" << endl;
        cout << "-------------------------------------------------------------------------------";
        skip = true;
        return;
    }
    ...
    {
        cout << "请输入命题变元,以逗号或空格分隔或不分隔,以回车结束:" << endl;
        getline(cin, input);
        for (int i = 0; i < input.length(); i++)
        {
